...
// ensure all coorinates are >= -180 && <= 180
const wrapAroundAntimeridian = (coordinates) => {
...
const subdivideTriangle = ({ properties: { id }, geometry: { coordinates: [[a, b, c]] } }) => {
  const ab = wrapAroundAntimeridian(midpoint(a, b).geometry.coordinates);
  const bc = wrapAroundAntimeridian(midpoint(b, c).geometry.coordinates);
  const ac = wrapAroundAntimeridian(midpoint(a, c).geometry.coordinates);
