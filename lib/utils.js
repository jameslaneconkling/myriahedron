...
const distance = exports.distance = ([lon1, lat1], [lon2, lat2]) => {
  var dLat = lat2 - lat1;
  var dLon = lon2 - lon1;
...
exports.midpoint = (degreesPoint1, degreesPoint2) => {
  ...
  var dist = distance(radianPoint1, radianPont2) / 2;
  var heading = bearing(radianPoint1, radianPont2);

  var lat2 = Math.asin(Math.sin(radianPoint1[1]) * Math.cos(dist) +
    Math.cos(radianPoint1[1]) * Math.sin(dist) * Math.cos(heading));
  var lon2 = radianPoint1[0] + Math.atan2(Math.sin(heading) * Math.sin(dist) * Math.cos(radianPoint1[1]),
    Math.cos(dist) - Math.sin(radianPoint1[1]) * Math.sin(lat2));
